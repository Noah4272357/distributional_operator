...
def validate_config(config: dict) -> None:
    ...
    pipeline = str(training_config.get("pipeline", "standard")).lower()
    if pipeline not in {"standard", "diffusion"}:
        raise ValueError("training.pipeline must be 'standard' or 'diffusion'")
    ...
    if early_stopping_config is not None:
        ...
        if not isinstance(early_stopping_config.get("enabled", False), bool):
            raise ValueError("training.early_stopping.enabled must be a boolean")
        if int(early_stopping_config.get("patience", 10)) < 1:
            raise ValueError("training.early_stopping.patience must be positive")
        if float(early_stopping_config.get("min_delta", 0.0)) < 0:
            raise ValueError(
